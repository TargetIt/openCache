...
| TC-ADDR-001 | F-CFG-009 | unit | Partial | `test/test_cache_whitebox.cc` | LINEAR/XOR/IPOLY/FERMI/CUSTOM set-index 在范围内；完整 golden 对照后续补充 |
...
| TC-TAG-007 | F-TAG-003 | unit | Planned | 后续 | 全 reserved line allocation fail 需要专门构造不导致默认回归不稳定的用例 |
| TC-TAG-008 | F-TAG-005 | unit | Implemented | `test/test_cache_whitebox.cc` | flush/invalidate 行为 |
...
| TC-WR-005 | F-WR-005 | unit | Implemented | `test/test_cache_whitebox.cc` | WA miss 产生 write/read/write-allocate |
| TC-WR-006 | F-WR-006, F-BLK-003 | unit | Implemented | `test/test_cache_whitebox.cc` | dirty eviction 产生 writeback event |
| TC-WR-007 | F-WR-007, F-BLK-004 | unit | Planned | 后续 | partial write readable/unreadable 精确状态需专用白盒访问器或更细粒度 oracle |
...
| TC-FUNC-001 | F-FUNC-001 | unit | Implemented | `test/test_cache_whitebox.cc` | DataStore 默认零、write/read |
...
| TC-COV-001 | F-COV-001 | coverage | Implemented | `coverage.sh` | 生成覆盖率报告；当前 line 58.48%、function 63.57%、branch 49.90% |
...
| step3-step7 | 通过，新增白盒覆盖关键架构风险；覆盖率需后续继续提升 | 通过，run.sh/CMake/coverage.sh 均纳入 | 通过，默认回归 12+10+12 全通过 | 通过，文档和追踪矩阵已刷新 |
